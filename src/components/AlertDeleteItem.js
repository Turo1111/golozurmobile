...
export default function AlertDeleteItem({ open, onClose, confirm, item }) {
    return (
        <ModalContainer
            openModal={open}
            onClose={onClose}
        ...
        >
            <Text style={{ textAlign: 'center', fontSize: 18 }}>
                {item?.descripcion ? `Se eliminará "${item.descripcion}" de la venta.` : 'Se eliminará el ítem de la venta.'}
            </Text>
